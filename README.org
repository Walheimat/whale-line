...
If you use strategy =tiered= (the default) you may want to customize
the defaults.

All segments belong to a tier. Segments belonging to a lower tier will
be removed if space is low. The strategy will always show the lowest
possible tier. The tiers are
...
If you use strategy =prioritize= you may want to customize the
defaults.
